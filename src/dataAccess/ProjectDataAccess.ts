...
function getAllProject(): IDockerProjectSummary[] {
  ...
  folders.forEach((folder) => {
    if (hasDockerComposeFile(folder)) {
      const dockerProject: IDockerProjectSummary = {
        ...
        folderName: folder,
        containers: Object.values(
          getProjectYaml(`${BASE_DIR}/${folder}/docker-compose.yml`).services
        )
          .map((container) => container.container_name)
          .filter((name) => name != undefined),
      };
      logger.info(
        `Found Docker project: ${dockerProject.name} in folder: ${folder}`
      );
  ...
  logger.info(
    `Completed fetching Docker projects. Total found: ${results.length}`
  );
  return results;
}
...
function getProject(projectFolder: string): IDockerProject {
  ...
  if (hasDockerComposeFile(projectFolder)) {
    const dockerProject: IDockerProject = {
      ...
      dockerCompose: getProjectYaml(
        `${BASE_DIR}/${projectFolder}/docker-compose.yml`
      ),
    };
    logger.info(
      `Successfully retrieved project: ${dockerProject.name} from folder: ${projectFolder}`
    );
...
function newProject(projectName: string): boolean {
  ...
  if (!fs.existsSync(projectPath)) {
    try {
      ...
      logger.info(
        `Successfully created project: ${projectName} at path: ${projectPath}`
      );
    } catch (err) {
...
function updateProjectName(projectName: string, newProjectName: string) {
  ...
  logger.info(
    `Attempting to rename project from ${projectName} to ${newProjectName}`
  );
  ...
  if (fs.existsSync(projectPath)) {
    ...
    try {
      ...
      logger.info(
        `Successfully renamed project from ${projectName} to ${newProjectName}`
      );
      return true;
    } catch (err) {
...
function updateProjectDockerCompose(
  ...
  logger.info(
    `Attempting to update docker-compose.yml for project: ${projectFolderName}`
  );
  ...
  try {
    ...
    logger.info(
      `Successfully updated docker-compose.yml for project: ${projectFolderName}`
    );
    return true;
  } catch (error) {
    logger.error(
      `Error updating docker-compose.yml for project ${projectFolderName}: ${error}`
    );
