...
async function getAllServiceStatus() {
    ...
    try {
        ...
        return allService.filter(x => x.container_name).map((service) => {
            ...
            logger.info(`Service: ${service.container_name}, State: ${serviceStatus.state}, Status: ${serviceStatus.status}`);
...
function getAllServices(): ServiceConfig[] {
    ...
    allProject.forEach((project) => {
        ...
        try {
            ...
            logger.error(`Error fetching details for project ${project.folderName}: ${error}`);
        }
    });
...
async function getContainerStats(containerId: string) {
    ...
    try {
        ...
        logger.info(`Container ${containerId} CPU: ${cpuUsage}% | Memory: ${memoryUsage} MB`);
        ...
        logger.error(`Failed to retrieve stats for container ${containerId}: ${error}`);
